...
import { Copy, Edit, Trash2, Eye, EyeOff, RefreshCw, Clock, AlertTriangle, History } from 'lucide-react';
...
const PasswordEntryCard: React.FC<PasswordEntryCardProps> = ({
  ...
  try {
    if (visiblePasswords.has(entry.id)) {
      ...
      isWeakPassword = strength.isWeak;
    }
  } catch {
        ...
        <div className="flex justify-between items-start">
          <div className="flex-1">
            <div className="flex items-center gap-2 flex-wrap">
              ...
              {isWeakPassword && visiblePasswords.has(entry.id) && (
                <span className="text-xs bg-orange-600/20 text-orange-400 px-2 py-1 rounded flex items-center gap-1">
            ...
            {isWeakPassword && visiblePasswords.has(entry.id) && (
              <div className="mt-2 bg-orange-900/20 border border-orange-500/50 rounded p-2">
                <div className="flex items-center gap-2">
                  <AlertTriangle className="w-4 h-4 text-orange-400" />
                  <span className="text-sm text-orange-400">
                    This password is weak. Consider updating it for better security.
                  </span>
                </div>
              </div>
            )}
